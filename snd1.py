...
<body>
  <div id="top">
    <b>SNS 테스트 인젝터</b> · 지도에서 한 점을 클릭하여 좌표를 선택하세요.
    <span style="margin-left:8px"><a href="/map" target="_blank">/map</a></span>
  </div>
  ...
  <div id="grid">
    <div>
      <div id="map"></div>
      <div class="row">
        <label style="min-width:80px;">위도(lat)</label><input id="lat" placeholder="37.5665" />
        <label style="min-width:80px;">경도(lon)</label><input id="lon" placeholder="126.9780" />
        <button id="gen" class="primary">curl 생성</button>
      </div>
    </div>
...
`curl -k -X POST https://localhost:8443/sns \\
  ...
  --data-binary $'${outerStr.replace(/'/g, "'\\''")}'`;
