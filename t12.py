...
<style>
  :root{
    /* 더 부드러운 톤 */
    --bg:#101725; --card:#162033; --accent:#5fb6ff; --accent-2:#86f7a6;
    --text:#ecf3ff; --muted:#b7c3d6; --border:rgba(255,255,255,.06);
    --shadow:0 10px 26px rgba(0,0,0,.28); --radius:16px;
  }
  *{box-sizing:border-box}
  ...
  body{
    ...
    background:
      ...
      radial-gradient(1200px 800px at 10% -10%, rgba(44,77,126,.35) 0%, transparent 60%),
      radial-gradient(900px 700px at 110% 20%, rgba(58,98,160,.30) 0%, transparent 55%),
      var(--bg);
  }
  /* 화면 꽉 차는 폭 */
  ...
  .status{font-size:14px; color:var(--muted); border:1px solid var(--border); padding:6px 10px; border-radius:999px; display:flex; align-items:center; gap:8px; backdrop-filter: blur(6px);}
  ...
  #home-card{ padding:24px; min-height: calc(100vh - 220px); }
  ...
  .btn{ display:inline-flex; align-items:center; gap:10px; padding:12px 16px; border-radius:12px; text-decoration:none; font-weight:700; border:1px solid var(--border); color:var(--text); background:#152238; transition: transform .08s ease, box-shadow .2s ease, background .2s ease;}
  ...
  .card2 .go{ margin-top:12px; display:inline-flex; align-items:center; gap:8px; padding:10px 12px; border-radius:10px; border:1px solid var(--border); background:#152238; color:var(--text); text-decoration:none; font-weight:700; }
...
<body>
  <div class="wrap">
    ...
    <div class="card" id="home-card">
      <section class="hero">
        <h1>드론연계 산불 화재 지원</h1>
        <p>현장의 드론/센서 이벤트를 실시간으로 수집·검증하고, 알림 대시보드와 지도로 가시화합니다.<br/>아래 메뉴에서 필요 기능으로 이동하세요.</p>
        <div class="cta">
      ...
      <div class="grid">
        ...
        <div class="card2 col-4">
          <h3>🗺️ Geo Map (/map)</h3>
          <p>메시지에서 추출한 위·경도 좌표를 지도에 표시합니다. 새 좌표가 들어오면 자동 갱신됩니다.</p>
    ...
    <footer>© 드론연계 산불 화재 지원 · 실시간 SNS 수신 · 대시보드 · 지도 시각화</footer>
  </div>
...
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>드론연계 산불 화재 지원 · 상태</title>
<style>
  :root{{ --bg:#101725; --card:#162033; --accent:#5fb6ff; --accent-2:#86f7a6; --text:#ecf3ff; --muted:#b7c3d6; --border:rgba(255,255,255,.06); --shadow:0 10px 26px rgba(0,0,0,.28); --radius:16px; }}
  *{{box-sizing:border-box}} html,body{{height:100%}}
  ...
  #status-card{{min-height: calc(100vh - 220px); display:flex; align-items:center; justify-content:center; text-align:center;}}
  .pill{{display:inline-flex; align-items:center; gap:8px; padding:8px 12px; border-radius:999px; background:#152238; border:1px solid var(--border);}}
...
<body>
  <div class="wrap">
    <div class="topbar">
      <div class="brand"><div class="brand-badge">🔥</div><div>드론연계 산불 화재 지원</div></div>
      <div>
    ...
    <footer style="margin-top:28px; padding-top:16px; border-top:1px solid var(--border); color:var(--muted); font-size:13.5px; text-align:center;">
...
<style>
  ...
  *{box-sizing:border-box} html,body{height:100%}
  ...
  .topbar{display:flex; align-items:center; justify-content:space-between; gap:12px; margin-bottom:16px;}
  .brand{display:flex; align-items:center; gap:12px; font-weight:800;}
  .brand-badge{width:38px; height:38px; border-radius:12px; background:linear-gradient(135deg, var(--accent), var(--accent-2));
    box-shadow:var(--shadow); display:flex; align-items:center; justify-content:center; color:#00101a; font-size:20px; font-weight:900;}
  .link{color:var(--accent); text-decoration:none; font-weight:700}
  ...
  #map-card{ padding:0; }
  #map{ width:100%; height: calc(100vh - 220px); border-radius:0 0 var(--radius) var(--radius); }
  .badge{ display:inline-block; padding:2px 8px; border-radius:10px; background:#1b2a44; color:var(--text); }
  select, button{ background:#152238; color:var(--text); border:1px solid var(--border); border-radius:8px; padding:6px 8px; }
</style>
</head>
<body>
  <div class="wrap">
    ...
    <div class="card" id="map-card">
      <div id="controls">
        <span class="badge">🗺️ Geo Map</span>
        <a class="link" href="/health" target="_blank" style="margin-left:auto">/health</a>
        <label>Rows</label>
  ...
  <script>
    ...
    async function pullAndRender() {
      ...
      try {
        ...
        data.forEach(p => {
          ...
          markers.addLayer(m); bounds.push([p.lat, p.lon]);
        });
        if (bounds.length > 0 && _shouldFit) {
          const b = L.latLngBounds(bounds); map.fitBounds(b.pad(0.2)); _shouldFit = false;
        }
    ...
    function schedule() {
      ...
      clearInterval(window.__timer); window.__timer = setInterval(pullAndRender, sec * 1000);
    }
...
<head>
  ...
  <meta charset="utf-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1"/>
  <title>드론연계 산불 화재 지원 · Dashboard</title>
  {%favicon%}
  {%css%}
  <style>
    :root{
    ...
    *{box-sizing:border-box} html,body{height:100%}
    ...
    .link{color:var(--accent); text-decoration:none; font-weight:700}
    .card{border:1px solid var(--border); background:var(--card); border-radius:var(--radius); box-shadow:var(--shadow); padding:18px;}
    #app-card{min-height: calc(100vh - 220px);}
  </style>
</head>
<body>
  <div class="wrap">
...
# 1) store(data) 채우기: 브라우저가 /api/notifications를 직접 fetch
dash_app.clientside_callback(
    """
    async function(n, rows){
      try{
        const r = await fetch('/api/notifications?limit=' + (rows || 100), {cache:'no-store'});
...
dash_app.clientside_callback(
    ...
    function(data){
      ...
      (data || []).forEach(function(row){
        ...
        let msg_str = (typeof msg === 'object' && msg !== null) ? JSON.stringify(msg) : String(msg||'');
        out.push({
          ts: row.ts || '',
          type: row.type || '',
          topic: row.topic || '',
