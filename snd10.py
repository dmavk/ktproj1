...
<div class="wrap">
  <h2>📸 사진 신고-1</h2>
  <p class="hint">지도에서 위치를 선택하고 사진 파일을 첨부한 뒤 <b>curl 생성</b> 버튼을 누르세요.</p>
...
<script>
  ...
  // Base64 유틸 (기존 로직 유지: 다른 전송 기능에 사용될 수 있음)
  ...
  function extractBase64(dataUrl){
    ...
    return m?{mime:m[1],b64:m[2]}:{mime:null,b64:null};
  }
  ...
  document.getElementById('genCurl').addEventListener('click', async ()=>{
    ...
    try{
      ...
      // 파일 이름/프리뷰 JSON 구성
      const epoch=Math.floor(Date.now()/1000); // 예: 1759751387
      const name=`${epoch}_${f.name}`;
      const preview={
        lat:Number(lat),
        lon:Number(lon),
        name:name,
