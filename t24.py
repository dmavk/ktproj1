...
def add_csp(resp: Response):
    if request.path.startswith("/map") or request.path.startswith("/img-map"):
        ...
        directives = [
            # 기본
            "default-src 'self' blob: data:",
            # ✅ 스크립트: Google Maps + deck.gl(unpkg)
            "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://maps.googleapis.com https://maps.gstatic.com https://unpkg.com",
            # ✅ 스타일: Google Maps 위젯이 gstatic CSS/폰트도 씀
            "style-src 'self' 'unsafe-inline' https://maps.gstatic.com https://fonts.googleapis.com https://unpkg.com",
            # ✅ 이미지: 지도 타일/아이콘/스프라이트
            "img-src 'self' data: blob: https://maps.googleapis.com https://maps.gstatic.com https://lh3.googleusercontent.com https://unpkg.com",
            # ✅ 네트워크: 타일/벡터/메타 호출
            "connect-src 'self' blob: data: https://maps.googleapis.com https://maps.gstatic.com https://unpkg.com",
            # ✅ 폰트/워커
